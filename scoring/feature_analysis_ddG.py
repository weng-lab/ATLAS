...
def plot_experiment_vs_predict(ddGs, predictions, subset_features, r):
	...
	ax.set_xlim([-10,10])
	ax.set_ylim([-6,6])
	ax.scatter(ddGs, predictions)
	xticks = map(int,ax.get_xticks().tolist())
	yticks = map(int, ax.get_yticks().tolist())
	ax.set_xticklabels(xticks, fontsize=16)
	ax.set_yticklabels(yticks, fontsize=16)
	ax.text(-8, 4, 'r = ' + str(round(r[0,1], 2)), fontsize= 20)
	ax.set_xlabel(r'Experimentally measured $\Delta\Delta$G (kcal/mol)', fontsize=16)
	ax.set_ylabel(r'Predicted $\Delta\Delta$G (kcal/mol)', fontsize=16)
